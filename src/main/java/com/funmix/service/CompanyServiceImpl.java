...
public class CompanyServiceImpl implements CompanyService {
	...
	private static final String	SQL_INSERT		= "INSERT INTO tcompany (companyname,phone,address,contact,contact_phone,memo,status) "
												+ "VALUES (?, ?, ?, ?, ?, ?, ?)";
	private static final String	SQL_QUERY		= "SELECT * FROM tcompany WHERE id = ? limit 1";
	private static final String	SQL_QUERY_ALL	= "SELECT * FROM tcompany";
	private static final String	SQL_QUERY_COUNT	= "SELECT count(*) as count FROM tcompany";
	
	private static final String	SQL_UPDATE		= "UPDATE tcompany SET ";
	private static final String	SQL_DELETE		= "DELETE FROM `tcompany` WHERE id = ?";	

	public CompanyServiceImpl(JsonObject config) {
		this(Vertx.vertx(), config);
	}

	public CompanyServiceImpl(Vertx vertx, JsonObject config) {
		this.vertx = vertx;
		this.config = config;
		...
		client.getConnection(connHandler(result, connection -> {
			...
			connection.queryWithParams(sqlQuery.toString(), queryparams, r -> {				
				if (r.failed()) {
					...
					jrs.put("data", r.result().getRows().stream().map(Company::new).collect(Collectors.toList()));	
	...
	@Override
	public Future<Optional<Company>> getOne(String id) {
		Future<Optional<Company>> result = Future.future();
		client.getConnection(connHandler(result, connection -> {
			connection.queryWithParams(SQL_QUERY, new JsonArray().add(id), r -> {
				if (r.failed()) {
					...
					if (list == null || list.isEmpty()) {
						...
						result.complete(Optional.of(new Company(list.get(0))));
					}
				}
				connection.close();
			});
	...
	@Override
	public Future<Optional<Company>> insert(JsonObject json) {		
		Future<Optional<Company>> result = Future.future();
		JsonArray params = new JsonArray();		
		params.add(json.getString("companyname"));
