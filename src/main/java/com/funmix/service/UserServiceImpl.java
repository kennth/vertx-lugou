...
public class UserServiceImpl implements UserService {
	...
	private static final String	SQL_INSERT		= "INSERT INTO tuser (email,username,phone,password,status) VALUES (?, ?, ?, ?, ?)";
	...
	private static final String	SQL_QUERY_COUNT	= "SELECT count(*) as count FROM tuser";
	
	private static final String	SQL_UPDATE		= "UPDATE tuser SET ";
	private static final String	SQL_DELETE		= "DELETE FROM `tuser` WHERE id = ?";
	
	private static final String	SQL_LOGIN	= "SELECT * FROM tuser where username = ? and password = ? and status=1 ";
	...
	@Override
	public Future<Optional<User>> login(String username,String password) {
		Future<Optional<User>> result = Future.future();
		client.getConnection(connHandler(result, connection -> {
			connection.queryWithParams(SQL_LOGIN, new JsonArray().add(username).add(password), r -> {
				if (r.failed()) {
					result.fail(r.cause());
				} else {
					...
					if (list == null || list.isEmpty()) {
						...
						connection.update("update tuser set token='" + token + "' where id=" + user.getId(), ur ->{
							if (ur.failed()) {
								result.fail(ur.cause());
							} else {
								user.setToken(token);
								result.complete(Optional.of(user));
							}
						});						
					}
				}
		...
		client.getConnection(connHandler(result, connection -> {
			...
			connection.queryWithParams(sqlQuery.toString(), queryparams, r -> {				
				if (r.failed()) {
					...
					jrs.put("data", r.result().getRows().stream().map(User::new).collect(Collectors.toList()));	
	...
	@Override
	public Future<Optional<User>> getOne(String userID) {
		Future<Optional<User>> result = Future.future();
		client.getConnection(connHandler(result, connection -> {
	...
	@Override
	public Future<Optional<User>> insert(JsonObject json) {
		Future<Optional<User>> result = Future.future();
		...
		params.add(phone);
		params.add(phone.substring(phone.length()-6));		
